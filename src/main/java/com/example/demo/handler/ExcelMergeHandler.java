...
public class ExcelMergeHandler implements CellWriteHandler {
        ...
        if (curRowIndex > mergeRowIndex) {
            for (int i = 0; i < mergeColumnIndex.length; i++) {
                if (curColIndex == mergeColumnIndex[i]) {
                    ExcelMergeHandler(writeSheetHolder, cell, curRowIndex, curColIndex);
                    break;
                }
            }
        }
     ...
     */
    private void ExcelMergeHandler(WriteSheetHolder writeSheetHolder, Cell cell, int curRowIndex, int curColIndex) {
